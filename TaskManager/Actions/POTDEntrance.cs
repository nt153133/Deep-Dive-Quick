...
namespace Deep2.TaskManager.Actions
{
    internal class POTDEntrance : ITask
    {
        ...
        private bool HasWindowOpen => DeepDungeonMenu.IsOpen || DeepDungeonSaveData.IsOpen;
        ...
        public async Task<bool> Run()
        {
            ...
            if (ContentsFinderConfirm.IsOpen)
            {
                ...
                Logger.Debug($"Entering POTD - Currently a Level {Core.Me.ClassLevel} {Core.Me.CurrentJob}");
        ...
        private async Task OpenMenu()
        {
            Logger.Verbose("Attempting to interact with: {0}",
                DataManager.GetLocalizedNPCName((int) Constants.CaptainNpcId));
...
//                }
                ...
                stop = Settings.Instance.SelectedLevel;
                _targetFloor = stop; //new FloorSetting { LevelMax = 60 };//stop;
