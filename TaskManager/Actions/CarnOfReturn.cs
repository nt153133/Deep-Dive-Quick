...
{
    internal class CarnOfReturn : ITask
    {
        private int Level;
        ...
        public void Tick()
        {
            ...
            if (Poi.Current != null && (Poi.Current.Type == PoiType.Kill ||
                                        Poi.Current.Type == (PoiType) PoiTypes.UseCarnOfReturn))
                return;
            ...
            if (PartyManager.AllMembers.Any(member => member.CurrentHealth == 0) && location != Vector3.Zero &&
                Level == DeepDungeonManager.Level) Poi.Current = new Poi(location, (PoiType) PoiTypes.UseCarnOfReturn);
        }
    }
}